...
# ========== 核心现代色调常量 ==========
COLOR_PANEL_BG = '#F8FAFC'  # 右侧面板背景（静谧蓝灰）
COLOR_CANVAS_BG = '#EDF2F7'  # 画布未加载图片时的底色（冷雾灰）
COLOR_TEXT_DARK = '#1E293B'  # 主文字色（深邃蓝黑，比纯黑更高级）
COLOR_TEXT_MUTED = '#64748B'  # 次要/状态栏文字（烟灰色）
COLOR_BORDER = '#E2E8F0'  # 极其精致的浅色描边线

# 警示色
COLOR_WARN_TEXT = '#EF4444'  # 优雅红（用于删除按钮文字）
...
def ui_init():
    ...
    left_frame = tk.Frame(root, bg=COLOR_PANEL_BG, bd=0, highlightthickness=1, highlightbackground=COLOR_BORDER)
    left_frame.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=20, pady=20)

    canvas = tk.Canvas(left_frame, width=canvas_width, height=canvas_height, bg=COLOR_CANVAS_BG, highlightthickness=0)
    canvas.pack(fill=tk.BOTH, expand=True, padx=6, pady=6)
    canvas.bind("<Button-1>", on_canvas_click)

    # ========== 右侧控制面板 ==========
    # 悬浮玻璃态面板效果
    right_frame = tk.Frame(root, bg=COLOR_PANEL_BG, width=280, bd=0, highlightthickness=1,
                           highlightbackground=COLOR_BORDER)
    right_frame.pack(side=tk.RIGHT, fill=tk.Y, padx=(0, 20), pady=20)
    right_frame.pack_propagate(False)

    # 1. 顶部标题
    title_label = tk.Label(right_frame, text=TEXTS[current_lang]['control_title'], font=('微软雅黑', 12, 'bold'),bg=COLOR_PANEL_BG, fg=COLOR_TEXT_DARK)
    title_label.pack(pady=(20, 10))

    # 2. 语言切换按钮 (极简现代幽灵按钮样式)
    lang_btn = tk.Button(right_frame, text=TEXTS[current_lang]['lang_switch'], font=('微软雅黑', 9), width=12, bg=COLOR_PANEL_BG, fg=COLOR_TEXT_MUTED, activebackground=COLOR_BORDER,  activeforeground=COLOR_TEXT_DARK, relief=tk.FLAT, bd=0, highlightthickness=1, highlightbackground=COLOR_BORDER, cursor='hand2')
    lang_btn.pack(pady=(0, 15))

    # 3. 点列表管理 (精细化线条白框)
    list_frame = tk.LabelFrame(right_frame, text=f"  {TEXTS[current_lang]['point_list_title']}  ", font=('微软雅黑', 9, 'bold'), bg=COLOR_PANEL_BG, fg=COLOR_TEXT_DARK, relief=tk.FLAT, bd=0, highlightthickness=1, highlightbackground=COLOR_BORDER)
    list_frame.pack(fill=tk.BOTH, expand=True, padx=15, pady=5)
    point_list_label = list_frame

    # 列表框：白底深字，极简科技感
    listbox = tk.Listbox(list_frame, height=10, font=('Consolas', 10),
                         bg='white', fg=COLOR_TEXT_DARK, relief=tk.FLAT, bd=0,
                         selectbackground='#3B82F6', selectforeground='white',  # 现代明亮蓝高亮
                         highlightthickness=0, activestyle='none')
    listbox.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=(12, 0), pady=12)

    # 滚动条极简处理
    scrollbar = tk.Scrollbar(list_frame, orient=tk.VERTICAL, command=listbox.yview, width=8, bd=0, elementborderwidth=0)
    scrollbar.pack(side=tk.RIGHT, fill=tk.Y, padx=(4, 6), pady=12)
    listbox.config(yscrollcommand=scrollbar.set)

    # ========== 统一按钮公共样式 (图片悬浮版) ==========
    # 通过 compound='center' 让文字完美浮在渐变图片正中央，同时增加 hand2 手势
    btn_img_style = {'font': ('微软雅黑', 9, 'bold'), 'fg': 'white', 'relief': tk.FLAT, 'bd': 0, 'compound': 'center',
                     'cursor': 'hand2'}
    btn_text_style = {'font': ('微软雅黑', 9, 'bold'), 'relief': tk.FLAT, 'bd': 0, 'cursor': 'hand2'}

    # 4. 基础操作按钮区
    btn_frame1 = tk.Frame(right_frame, bg=COLOR_PANEL_BG)
    btn_frame1.pack(pady=15)

    # 导入图片作为背景，高端视觉的核心
    open_btn = tk.Button(btn_frame1, image=img_btn_blue, text=TEXTS[current_lang]['open_image'], command=select_image, **btn_img_style)
    open_btn.pack(pady=5)

    # 次要危险按钮采用高级幽灵按钮（白底红字加描边），不破坏整体高级感
    delete_btn = tk.Button(btn_frame1, text=TEXTS[current_lang]['delete_point'], command=delete_selected_points,
                           bg='white', fg=COLOR_WARN_TEXT, activebackground='#FEF2F2',
                           highlightthickness=1, highlightbackground=COLOR_BORDER, width=17, pady=4, **btn_text_style)
    delete_btn.pack(pady=5)

    clear_btn = tk.Button(btn_frame1, text=TEXTS[current_lang]['clear_all'], command=clear_all_points,
                          bg='white', fg=COLOR_TEXT_MUTED, activebackground='#F8FAFC',
                          highlightthickness=1, highlightbackground=COLOR_BORDER, width=17, pady=4, **btn_text_style)
    clear_btn.pack(pady=5)

    # 5. 核心算法处理区 (双重渐变光影)
    proc_frame = tk.LabelFrame(right_frame, text=f"  {TEXTS[current_lang]['process_title']}  ",
                               font=('微软雅黑', 9, 'bold'),
                               bg=COLOR_PANEL_BG, fg=COLOR_TEXT_DARK, relief=tk.FLAT, bd=0, highlightthickness=1,
                               highlightbackground=COLOR_BORDER)
    proc_frame.pack(fill=tk.X, padx=15, pady=15)
    process_label = proc_frame

    skew_btn = tk.Button(proc_frame, image=img_btn_cyan, text=TEXTS[current_lang]['skew_correct'], command=skew_correction, **btn_img_style)
    skew_btn.pack(pady=8, padx=10)
    ...
    status_frame.pack(fill=tk.X, side=tk.BOTTOM, pady=(0, 10))

    # 浅蓝色的小指示灯 (•)
    status_label = tk.Label(status_frame, text=f"●  {TEXTS[current_lang]['status_ready']}", font=('微软雅黑', 9), bg=COLOR_PANEL_BG, fg=COLOR_TEXT_MUTED)
